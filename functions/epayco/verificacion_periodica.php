<?php
// Este script se ejecutará cada 30 minutos utilizando un trabajo cron, con ssl para que no genere error
...
<?php
// Este script se ejecutará cada 30 minutos utilizando un trabajo cron sin ssl
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        // Crear un contexto de flujo con verificación SSL deshabilitada
        ...
        // Realiza una solicitud a ePayco para verificar el estado actual
        $urlVerificacion = "https://secure.epayco.co/validation/v1/reference/" . $id_ref_payco;
        $response = file_get_contents($urlVerificacion, false, $context);
        $responseData = json_decode($response, true);
